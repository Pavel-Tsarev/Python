...
def iozh(x, y, s, k, colour, colour1, colour2):
    ...
    k = 0, if hedgehog must look to the left
    colour - colour of the hedgehog's body and head
    colour1 - colour of hedgehog's apple
    colour2 - colour of the hedgehog's mushroom
    '''
    ellipse(screen, colour, (x, y, 100*s, 70*s))                      #body
    ellipse(screen, colour, (x + 80*s, y + 55*s, 20*s, 10*s))         #head
    ellipse(screen, colour, (x, y + 55*s, 20*s, 10*s))                #legs
    ellipse(screen, colour, (x - 10*s, y + 40*s, 20*s, 10*s))
    ellipse(screen, colour, (x + 92*s, y + 45*s, 10*s, 10*s))
    ellipse(screen, colour, (x + 80*s, y + 25*s, 40*s, 20*s))
    circle(screen, (0, 0, 0), (x + 120*s, y + 35*s), 2*s)                   #nose
    circle(screen, (0, 0, 0), (x + 110*s, y + 30*s), 3*s)                   #eyes
    ...
    for i in range(10*s, 95*s, 10*s):
        for k in range(30*s, 65*s, 10*s):
            if not ((i > 70*s) and (k > 55*s)):
                koluchka(x + i, y + k, s)
    apple(x, y, s, colour1)
    apple(x + 10*s, y, s, colour1)
    grib (x + 50*s, y, s, colour2)
    for i in range(10*s, 95*s, 10*s):
        for k in range(30*s, 65*s, 10*s):
            if not ((i > 79*s) and (k > 55*s)):
                koluchka(x + i, y + k, s)

def grib(x, y, s, colour):
    '''
    ...
    ellipse(screen, colour, (x - 20*s, y, 70*s, 20*s))                      #mushroom's hat
    ellipse(screen, (255, 255, 255), (x - 5*s, y + 5*s, 10*s, 3*s))         #mushroom's dots
...
iozh(300, 600, 1, 1, (0, 0, 150), (0, 255, 0), (255, 0, 0))
...
grib(500, 930, 1, (255, 0, 0))
grib(500, 930, 1.1, (255, 0, 255))
grib(600, 930, 1, (255, 0, 0))
grib(650, 930, 0.8, (255, 255, 0))
grib(701, 930, 1.3, (255, 0, 0))
